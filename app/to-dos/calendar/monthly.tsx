  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col" style={{ background: COLORS.background, color: COLORS.primaryText }}>
      <header className="flex items-center justify-between px-8 py-6 border-b border-border bg-card/80 shadow-card sticky top-0 z-20">
        ...
        <button
          className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-xl shadow transition-all text-base"
      ...
      <main className="flex-1 flex flex-col items-stretch p-8">
        <div className="w-full max-w-6xl mx-auto bg-card rounded-xl shadow-card p-6 min-h-[600px] h-[80vh] flex flex-col justify-stretch overflow-x-auto">
          {/* Days header */}
          <div className="grid grid-cols-7 mb-2">
            {DAYS.map(day => (
              <div key={day} className="text-center text-sm font-semibold pb-2" style={{ color: COLORS.secondaryText }}>{day}</div>
            ))}
          </div>
          {/* Month grid */}
          <div className="grid grid-cols-7 gap-px bg-border rounded-xl overflow-hidden" style={{ background: COLORS.border }}>
            {grid.flat().map((date, idx) => {
              ...
              return (
                <div
                  ...
                  className={`relative min-h-[90px] h-[110px] p-2 flex flex-col rounded-lg transition-colors duration-200 cursor-pointer ${isCurrentMonth ? 'bg-surface' : 'bg-background/60'} ${isToday ? 'ring-2 ring-blue-500' : ''}`}
                  style={{ background: isCurrentMonth ? COLORS.surface : COLORS.background, border: `1px solid ${COLORS.border}` }}
                  onClick={() => handleDayClick(date)}
                >
                  <div className={`text-xs font-semibold mb-1 ${isCurrentMonth ? '' : 'opacity-40'}`}>{date.getDate()}</div>
                  <div className="flex flex-col gap-1 flex-1 overflow-y-auto">
                    {dayTasks.map(task => (
                      <div
                        key={task.id}
                        className="rounded-md px-2 py-1 text-xs font-medium truncate shadow-card mb-1 hover:scale-105 hover:shadow-lg transition"
